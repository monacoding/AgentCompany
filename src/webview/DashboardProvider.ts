...
export class DashboardProvider implements vscode.WebviewViewProvider {
  ...
  private async handleReleaseRefresh(): Promise<void> {
    this.view?.webview.postMessage({
      type: 'releaseStatus',
      payload: { status: 'running', step: 'bump', message: '릴리스 준비…' },
    });
    ...
    try {
      await vscode.window.withProgress(
        ...
        async (progress) => {
          const result = await this.service.runReleaseAndReload((p) => {
            progress.report({ message: p.message });
            this.view?.webview.postMessage({
              type: 'releaseStatus',
              payload: { status: 'running', ...p },
            });
          });
  ...
  private async handleMessage(message: { type: string; payload?: unknown }): Promise<void> {
    ...
    switch (message.type) {
      ...
      case 'getProjectDetail': {
        const { sessionId } = message.payload as { sessionId: string };
        const detail = this.service.getProjectDetail(sessionId);
        this.view?.webview.postMessage({ type: 'projectDetail', payload: detail });
        break;
      }
