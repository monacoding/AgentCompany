...
def main() -> int:
    ...
    html_pages = [("대학수학능력시험", fetch_html(LIST_URL))]
    if args.include_mock:
        html_pages.append(("수능모의평가", fetch_html(MOCK_LIST_URL)))

    downloaded: list[str] = []
    for board_name, html in html_pages:
        board_dir = out_dir / board_name
        board_dir.mkdir(parents=True, exist_ok=True)
        for entry in parse_entries(html, subjects):
            ...
            try:
                ...
                print(f"OK  {board_name}/{dest.name}")
